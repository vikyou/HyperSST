...
class BaseDataset(object):
    def __init__(self, type: str, name: str, device: str = 'cpu'):
        ...
        self.preprocess_dataset2()

    def load_dataset(self):
        with open(osp.join(self.dataset_dir, 'features1.pickle'), 'rb') as f:
            self.features = pickle.load(f)
        with open(osp.join(self.dataset_dir, 'hypergraph1.pickle'), 'rb') as f:
            self.hypergraph = pickle.load(f)
        with open(osp.join(self.dataset_dir, 'features2.pickle'), 'rb') as f:
            self.features2 = pickle.load(f)
        with open(osp.join(self.dataset_dir, 'hypergraph2.pickle'), 'rb') as f:
            self.hypergraph2 = pickle.load(f)
    ...
    def preprocess_dataset(self):
        ...
        self.features = torch.as_tensor(self.features.toarray(), dtype=torch.float32)
        self.features2 = torch.as_tensor(self.features2.toarray(), dtype=torch.float32)
    ...
    def preprocess_dataset2(self):
        edge_set = set(self.hypergraph2.keys())
        edge_to_num = {}
        num_to_edge = {}
        num = 0
        for edge in edge_set:
        ...
        processed_hypergraph = {}
        for edge in edge_set:
            nodes = self.hypergraph2[edge]
            processed_hypergraph[edge_to_num[edge]] = nodes
            for node in nodes:
                incidence_matrix.append([node, edge_to_num[edge]])

        self.processed_hypergraph2 = processed_hypergraph
        self.hyperedge_index2 = torch.LongTensor(incidence_matrix).T.contiguous()

        self.num_nodes2 = int(self.hyperedge_index2[0].max()) + 1
        self.num_edges2 = int(self.hyperedge_index2[1].max()) + 1
        self.edge_to_num2 = edge_to_num
        self.num_to_edge2 = num_to_edge

        print('=============== Dataset Stats ===============')
        print(f'dataset type: {self.type}, dataset name: {self.name}')
        print(f'features size: [{self.features2.shape[0]}, {self.features2.shape[1]}]')
        print(f'num nodes: {self.num_nodes2}')
        print(f'num edges: {self.num_edges2}')
    ...
    def to(self, device: str):
        ...
        self.features2 = self.features2.to(device)
    ...
    def generate_random_split(self, train_ratio: float = 0.1, val_ratio: float = 0.1,
                              seed: Optional[int] = None, use_stored_split: bool = False):
        if use_stored_split:
